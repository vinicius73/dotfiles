...
fish_functions="$repo_root/home/dot_config/fish/functions"
fish_load_environment="set -a fish_function_path '$fish_functions'; source '$fish_functions/dotfiles_load_environment.fish'; dotfiles_load_environment"
...
output=$(HOME="$home" USER=tester PATH="$temporary_dir/bin:$PATH" fish --no-config -c "$fish_load_environment; printf '%s:%s' \$SAFE_VALUE \$KEYCHAIN_VALUE")
...
output=$(HOME="$home" USER=tester KEYCHAIN_VALUE=existing-value PATH="$temporary_dir/bin:$PATH" fish --no-config -c "$fish_load_environment; printf '%s' \$KEYCHAIN_VALUE")
[ "$output" = 'existing-value' ] || fail "Fish existing environment value did not take precedence over Keychain"
